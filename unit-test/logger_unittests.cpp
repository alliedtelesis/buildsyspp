...
TEST_CASE_METHOD(LoggerTestsFixture, "Test Logger program_output() with coloured output",
                 "")
{
	Logger logger;
	logger.force_colour_output(true);

	std::stringstream buffer;
	std::cout.rdbuf(buffer.rdbuf());

	std::string test_string("error: This is a test log message");
...
TEST_CASE_METHOD(LoggerTestsFixture, "Test Logger program_output() without coloured output",
                 "")
{
	Logger logger;
	logger.force_colour_output(false);
